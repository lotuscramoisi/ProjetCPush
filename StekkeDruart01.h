...
void affichageJoggeur(joggeur *debJ,int nbJoggeur)
{
	...
	printf("Liste des joggeurs inscrits a l\'ACRHO\n");
	printf("_______________________________________________________________________________________________________________\n");
	printf("|No |    Nom     |   Prenom   |    Club    | Date Nais  | Dossart |Sexe |Nb Course|Score Total|Age |Categorie |\n");
	printf("|___|____________|____________|____________|____________|_________|_____|_________|___________|____|__________|\n");
...
void enregistrementCourse(joggeur *debJ, int nbJoggeur)
{
	...
	if(strcmp(typeCourse,"challenge")==0 || strcmp(typeCourse,"patronne")==0)	
	{
		...
		jEnCours=debJ;
		printf("\n%ccriture dans le fichier r%cussie\n\n",144,130);
		for(i=1; jEnCours!=NULL ; i++)
		{
...
void courseChallenge(FILE *fCourse,joggeur *debJ, int nbJoggeur)
{
	...
	printf("|Place |   Dossard  |   Nom      |   Prenom     |    Club    |  Temps |Vitesse     |Score   |\n");
	printf("|______|____________|____________|______________|____________|________|____________|________|\n");
...
void coursePatronne(FILE *fCourse,joggeur *debJ, int nbJoggeur)
{
	...
	int i,j, k , nbParticipant=1, tmp, vitessemin;
	float vitesse=0, vitessesec;	
	char sexejog;
	const char* categorie;
	...
	for(j=1;j<=nbParticipant;j++)
	{
		...
		for(i=1;i<=nbJoggeur;i++)
		{
			...
			if(jEnCours->dossart==cEnCours->dossartC)
			{
				...
				jEnCours->nbCourses+=1;
				tmp=calculPoint(nbParticipant,cEnCours,nbKilometre);
				
				printf("|  %3d | %07d    | %-11s|%-11s   |%-11s |%01dh%02dm%02ds|  %02d\'%02.0f/km  | %07d|\n",cEnCours->place , jEnCours->dossart, jEnCours->nom, jEnCours->prenom, jEnCours->club, cEnCours->heures, cEnCours->minutes, cEnCours->secondes, vitessemin, vitessesec, tmp);
...
{
	int menu=9, i, nbClassementGlobal, nbAfficher, place, age;
	const char* categorie;
	...
	while(menu != 0){
		...
		if(menu == 1)
		{
			...
			printf("|-------------------------------------------------------------------------------------------|\n");
			printf("|  Place  |     Nom            |     Prenom          |     ScoreTotal      |   Cat%cgorie    |\n",130);
			printf("|---------|--------------------|---------------------|---------------------|----------------|\n");
			...
			for(i=1;jEnCours!=NULL;i++)
			{
				if(jEnCours->nbCourses>=10)
				{
					...
					age = calculerAge(jEnCours);
					categorie = setCategorie(age, jEnCours->sexe);
					printf("|  %02d     |     %-15s|     %-15s |     %7d         |       %s       |\n", place,jEnCours->nom,jEnCours->prenom,jEnCours->scoreTotal, categorie);
				}
		...
		if(menu == 3)
		{
			...
			printf("Voici les diff%crentes cat%cgories disponibles:\n\n",130,130);
			printf("	|-------------------------|\n");
			...
			printf("	| 70  |   V3    |   A3    |\n");	
			printf("	|-------------------------|\n");
			printf("\nVeuillez entrer le nom de la categorie souhaitee\n");		
...
//Fonction de suppression d'un joggeur
void suppressionJoggeur(joggeur *debJ, int joggeurSupp){
	FILE *fECRITURE;
	fECRITURE = fopen("StekkeDruart01.dat", "w");
	int i, j;
	joggeur *jEnCours, *precedent;
	if(joggeurSupp == 1){
		debJ = debJ->suivant;
	}
	else{
		jEnCours = debJ;
		for(i=1; i< joggeurSupp ; i++){
			precedent = jEnCours;
			jEnCours = jEnCours->suivant;
		}
		precedent->suivant = jEnCours->suivant;
		if(jEnCours->suivant == NULL){
			precedent->suivant = NULL;
		}
	}
		//Ré écriture des joggeurs dans le fichier dat pour prendre en compte la suppression
		jEnCours=debJ;
		printf("\n%ccriture dans le fichier r%cussi\n\n",144,130);
		for(i=1; jEnCours!=NULL ; i++)
		{
			fprintf(fECRITURE, "%-12s %-12s %-12s %02d/%02d/%4d %07d %c %02d %02d %04d", jEnCours->nom, jEnCours->prenom, jEnCours->club, jEnCours->datenjour, jEnCours->datenmois, jEnCours->datenannee, jEnCours->dossart, jEnCours->sexe, jEnCours->nbCoursesPatronnes, jEnCours->nbCoursesChallenges, jEnCours->scoreTotal);
